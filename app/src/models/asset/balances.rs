...
use crate::{error::KoiError, models::{account::Account, asset::{Asset, identity::AssetIdentity}, network::identity::NetworkIdentity}, state::AppState};
...
impl Asset {
    pub async fn fetch_balance(
        ...
        let address = account.metadata.unwrap_address().ok_or(KoiError::Internal("Account has no address".to_string()))?;

        match asset_identity {
            AssetIdentity::Native(network_identity) => {
                if !account.networks.contains(network_identity) {
                    Err(KoiError::Internal("Account is not on the network".to_string()))
                } else {
            ...
            AssetIdentity::Fiat(_code) => Err(KoiError::Internal("Fiat balances are not supported".to_string())),
        }
    }
    ...
    pub async fn fetch_native_balance(
        ...
        let balance = provider.get_balance(address).await.map_err(|e| KoiError::Internal(format!("Failed to fetch balance: {}", e)))?;
        Ok(balance)
    }
}
