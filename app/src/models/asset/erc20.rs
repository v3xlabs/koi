...
impl Asset {
    pub async fn fetch_erc20_metadata(
        ...
        let (network_identity, address) = asset_identity.unwrap_address().ok_or(
        ...
        let token_name = contract
            ...
            .call()
            .await
            .map_err(|e| KoiError::Internal(format!("Failed to fetch name: {}", e)))?;
        let token_symbol = contract
            .symbol()
            .call()
            .await
            .map_err(|e| KoiError::Internal(format!("Failed to fetch symbol: {}", e)))?;
        let token_decimals = contract
            .decimals()
            .call()
            .await
            .map_err(|e| KoiError::Internal(format!("Failed to fetch decimals: {}", e)))?;
    ...
    pub async fn fetch_erc20_balance(
        ...
        let (network_identity, token_address) = asset_identity.unwrap_address().ok_or(
            KoiError::Internal("Asset identity is not an ERC20 address".to_string()),
        )?;
        let rpc = state
            .networks
