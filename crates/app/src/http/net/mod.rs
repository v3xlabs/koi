...
impl NetworkApi {
    ...
    ///
    /// GET /api/net/:network_identity/rpc
    #[oai(
        path = "/net/:network_identity/rpc",
        method = "get",
        tag = "ApiTags::Network"
    )]
    async fn get_network_rpc_stats(
        &self,
        auth: Auth,
        state: Data<&AppState>,
        network_identity: Path<NetworkIdentity>,
    ) -> Result<Json<RpcPoolStats>> {
        let _auth_data = auth.unwrap()?;

        Ok(Json(state.networks.get_pool(&network_identity).snapshot()))
    }
    ...
    async fn update_network_endpoint_by_id(
        ...
        state
            ...
            .get_rpc(&endpoint_identity, &state)
            .await
    ...
    async fn delete_network_endpoint_by_id(
        ...
        state
            .networks
            .get_pool(&network_identity.0)
            .remove_endpoint(&endpoint_identity.0);
    ...
    async fn get_network_endpoint_status(
        ...
        let rpc = pool.get_rpc(&endpoint_identity, &state).await;
